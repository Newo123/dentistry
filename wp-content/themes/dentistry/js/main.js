document.addEventListener('DOMContentLoaded', () => {
	...
	function checkForm() {
		function validateForm(e) {
			...
			const modalFormError = document.querySelector('.modal-form-error');
			...
			if (isValid) {
				(async () => {
					...
					const hash = await fetch(e.target.dataset.hash).then(res =>
						res.text(),
					);
					if (hash) {
						formData.set('hash', hash);
					}
					const request = await fetch(e.target.dataset.send, {
						method: 'POST',
						body: formData,
					});
					...
					if (response?.success) {
						...
						modalFormError.classList.add('active');
					}
				})();
			}
		}
